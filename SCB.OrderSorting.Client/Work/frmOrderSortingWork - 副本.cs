...
namespace SCB.OrderSorting.Client
{
    public partial class frmOrderSortingWork : Form
    {
        ...
        private bool IsCheckGratingSuspending
        {
            ...
            {
                return _IsError
                    || _IsCheckGratingRunning || _IsCheckButtonRunning;
            }
        }
        /// <summary>
        /// 是否暂停执行CheckButton
        ...
        private bool IsCheckButtonSuspending
        {
            get
            {
                return _IsBlocked || _IsRepeatError || _TargetLattice != null || _ResultLattice != null
                  || _IsCheckGratingRunning || _IsCheckButtonRunning || _IsCheckUnblockRunning || _IsCheckBlockRunning;
            }
        }
        /// <summary>
        /// 是否暂停执行CheckBlock
        ...
        private void txtOrderId_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode != Keys.Enter)
                return;
            if (string.IsNullOrWhiteSpace(txtOrderId.Text) || _IsBlocked)
            {
                txtOrderId.Text = "";
                return;
            }
                ...
                try
                {
                    ...
                    foreach (var slave in _SlaveConfig)
                    {
                        ...
                        for (int i = 0; i < registersGrating.Length; i++)
                        {
                            ...
                            //如果没有扫描且投递了，则提示 
                            if (_TargetLattice == null&& registersGrating[i]<1000) {
                                Thread.Sleep(1000);
                                var registersGratingNext = OrderSortService.ReadGratingRegisters(slave.SlaveAddress);
                                if (registersGratingNext[i] >= 1000) {
                                ...
                                if (OrderSortService.SetLED(_ResultLattice, 1))
                                {
                                    ...
                                    Invoke((MethodInvoker)delegate ()
                                    {
                                        if (MessageBox.Show("请先扫描后再投递！", "警告", MessageBoxButtons.OK) == DialogResult.OK)
                                        {
                                            if (OrderSortService.SetLED(_ResultLattice, 3))
                                            {
                                                //  OrderSortService.ReSetWarningLight();
                                                OrderSortService.ReSetGratingOrButton(1);
                                                _ButtonList.FirstOrDefault(b => b.TabIndex == _ResultLattice.ID).BackColor = GetColor(3);
                                                _IsError = false;
                                                _IsCheckGratingRunning = false;
                                            }
                                        }
