...
namespace SCB.OrderSorting.Client
{
    public partial class frmOrderSortingWorkNew : Form
    {
       // Stopwatch sw=new Stopwatch();
        ...
        #endregion
        #region 线程
        List<KeyValuePair<byte,Task>>  _ThreadList = new List<KeyValuePair<byte, Task>>();
        List<ThreadManage> _ThreadManageList = new List<ThreadManage>();
        ...
        private void ThreadRun()
        {
            try
            {
                ...
                foreach (SlaveConfig sc in _SlaveConfig)
                {
                    ...
                    {
                        SlaveConfig = sc,
                        QueueWrite = new System.Collections.Concurrent.BlockingCollection<Model.ThreadWriteMsg>()//new Queue<Model.ThreadWriteMsg>()
                    };
                    _ThreadManageList.Add(ThreadManage);
                    _ThreadList.Add(
                    ...
                    _ThreadList.Add(
                        new KeyValuePair<byte, Task>(
                            sc.SlaveAddress,
                            Task.Factory.StartNew(ThreadWrite, ThreadManage)
                        )
                     );
                }
            }
        ...
        private void ThreadWrite(object obj)
        {
            ...
            {
                ThreadManage ThreadManage = (ThreadManage)obj;
                foreach (var WriteMsg in ThreadManage.QueueWrite.GetConsumingEnumerable())
                {
                ...
                //while (!_ThreadStop)
                //{
                //    if (ThreadManage.QueueWrite.Count > 0)
                //    {
                //        lock (asyncLock)
                //        {
                //            //可优化：当队列没有数据时，可以通过AutoResetEvent进行线程暂停，在必要时即队列有数据时在启动线程
                //            ThreadWriteMsg WriteMsg = ThreadManage.QueueWrite.Take();
                //            if (WriteMsg != null)
                //            {
                //                ThreadWriteMsg(WriteMsg);
                //                UpdateSortStatus(WriteMsg.FinishStatus);
        ...
        private void 开始分拣ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                //    //sw.Restart();
                //   // SaveErrLogHelper.SaveErrorLog($"{i}开始写LED", sw.ElapsedMilliseconds.ToString());
                //    OrderSortService.TCPPortManage.SetLED(14, 1, 1);
                //  //  SaveErrLogHelper.SaveErrorLog($"{i}结束写LED", sw.ElapsedMilliseconds.ToString());
                //}
                //return;
                #endregion
        ...
        private void txtOrderId_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                {
                    SetTipMsg(string.Format("未找到相匹配的扫描抢，请检查扫描抢是否设置正确！"));
                    ClearOrderId();
                    return;
                }
                //禁止连续扫同一订单
                ...
                {
                    SetTipMsg(string.Format("您已扫描过订单号（{0}），无需重复扫描,请将订单投递至对应的格口！", orderID));
                    //防呆
                ...
                {
                    SetTipMsg(string.Format("之前扫描的订单号（{0}）还未投递,请先投递后再扫描其他订单！", ThreadSortOrder.SortOrderNo));
                    ClearOrderId();
                    return;
                }
                //扫描的订单是上次成功投递的订单即属于重复投递
                if (ThreadSortOrder.SortStatus == SortStatus_Enum.Success && ThreadSortOrder.SortOrderNo == orderID)
                {
                    SetTipMsg(string.Format("该订单({0})已投递,请将投递错误的订单取出重新扫描！", ThreadSortOrder.SortOrderNo));
                    ClearOrderId();
                ...
                if (ThreadSortOrder.SortStatus == SortStatus_Enum.RepeatError)
                {
                    foreach (var TargetLattice in ThreadSortOrder.TargetLattice)
                    {
                        ...
                        {
                            SetTipMsg(string.Format("此订单:{0}已入格口,请找出错误订单重新扫描！", orderID));
                            ClearOrderId();
                            return;
                        }
                    }
                ...
                {
                    SetTipMsg(string.Format("订单({0})投错格口，请先将其取出并重新扫描！", ThreadSortOrder.SortOrderNo));
                    ClearOrderId();
                    return;
                }
                //格口格档
                ...
                {
                    SetTipMsg(string.Format("格口被阻挡，请先将其推入后再扫描！", ThreadSortOrder.SortOrderNo));
                    ClearOrderId();
                    return;
                }
                ...
                {
                    SetTipMsg(string.Format("请等待！"));
                    ClearOrderId();
                    return;
                }
                if (ThreadSortOrder.IsStop && ThreadSortOrder.SortOrderNo == orderID && ThreadSortOrder.SortStatus == SortStatus_Enum.RepeatError)
                {
                    ThreadSortOrder.IsStop = false;
                    ClearOrderId();
                }
                ...
                {
                    SetTipMsg(string.Format("获取订单数据失败,订单号：{0}，{1}}！", orderID, GetOrderInfoResult));
                    ClearOrderId();
                    return;
                }
                //获取目标格口
                ...
                {
                    SetTipMsg(string.Format("获取订单对应的格口信息失败：订单号:{0}！", orderID));
                    ClearOrderId();
                    return;
                }
                ////禁止扫其它扫描枪进行中的格口
                ...
                {
                    SetTipMsg(string.Format("该订单（{0}）的格口正在工作中，请稍后在扫描！", orderID));
                    ClearOrderId();
                    return;
                }
                //格口未满的格口
                ...
                {
                    SetTipMsg(string.Format("订单号({0})对应的格口已满", ThreadSortOrder.OrderInfo.OrderId));
                    ClearOrderId();
                    SetQueueLED(ThreadSortOrder.TargetLattice, LED_Enum.YellowFlash);
                    return;
                }
                #endregion
               // SaveErrLogHelper.SaveErrorLog("验证", sw.ElapsedMilliseconds.ToString());
                ...
                {
                    //  SaveErrLogHelper.SaveErrorLog("保存进数据库", sw.ElapsedMilliseconds.ToString());
                ...
                {
                    SetTipMsg(string.Format("创建扫描记录失败，订单号：{0}", orderID));
                    ClearOrderId();
                }
            }
            catch (Exception ex)
            {
                SaveErrLogHelper.SaveErrorLog(string.Empty, ex.ToString());
                ClearOrderId();
                // SetTipMsg(string.Format(ex.Message));
                MessageBox.Show(ex.Message);
            }
        }
        #endregion
        ...
        {
            _ThreadStop = true;
            _ThreadList.Select(o => o.Value).ToList().ForEach(
                p => p.Dispose()
             );
        }
        #endregion
                ...
                switch (CounterType)
                {
                    case ReSetCounterType_Enum.Grating:
                        ...
                        //}
                         if (slave == null)
                        {
                            OrderSortService.TCPPortManage.ClearGrating();
                        }
                        ...
                        {
                             OrderSortService.TCPPortManage.ClearGrating(slave.SlaveAddress);
                        }
                        break;
                    case ReSetCounterType_Enum.Button:
                        if (slave == null)
                        {
                            OrderSortService.TCPPortManage.ClearButton();
                        }
                        else if (Index != ushort.MaxValue)
                        {
                            OrderSortService.TCPPortManage.ClearButton(slave.SlaveAddress, Index);
                        }
                        else
                        {
                            OrderSortService.TCPPortManage.ClearButton(slave.SlaveAddress);
                        }
                        break;
        ...
        {
            OrderSortService.TCPPortManage.SetLED(GetSlaveAddress(LatticeSetting.CabinetId), LatticeSetting.LEDIndex, (ushort)LED);
        }

      
        private void SetPortLED(int cabinetId, int LEDIndex, LED_Enum LED)
        {
            OrderSortService.TCPPortManage.SetLED(GetSlaveAddress(cabinetId), LEDIndex, (ushort)LED);
        }
        private void SetPortLED(int cabinetId, List<LEDChange> LEDChangeList)
        {
            OrderSortService.TCPPortManage.SetLED(GetSlaveAddress(cabinetId), LEDChangeList.Select(o => new KeyValuePair<int, ushort>(o.LEDIndex, (ushort)o.LED)).ToList());
        }
        private void SetPortWarningLight(WarningLight_Enum WarningLight, LightOperStatus_Enum LightOperStatus)
        {
            switch (WarningLight)
            {
                case WarningLight_Enum.Green:
                    OrderSortService.TCPPortManage.SetWarningLightGreen((ushort)LightOperStatus);
                    break;
                case WarningLight_Enum.Red:
                    OrderSortService.TCPPortManage.SetWarningLightRed((ushort)LightOperStatus);
                    break;
                case WarningLight_Enum.WarningSound:
                    OrderSortService.TCPPortManage.SetWarningTwinkle((ushort)LightOperStatus);
                    break;
                case WarningLight_Enum.Yellow:
                    OrderSortService.TCPPortManage.SetWarningLightYellow((ushort)LightOperStatus);
                    break;
                default:
                    break;
            }
        ...
        private void SetQueueLED( List<LatticeSetting> LatticeSettingList, LED_Enum status, int milliseconds = 0, FinishStatus FinishStatus = null)
        {
                ...
                LatticeSettingFormat.ForEach(o =>
                   _ThreadManageList.Find(p => p.SlaveConfig.CabinetId == o.CabinetId).QueueWrite.Add(
                     new ThreadWriteMsg
                     {
                         FinishStatus = FinishStatus,
        ...
        private void SetQueueCount(ReSetCounterType_Enum status, SlaveConfig slave, ushort Index, FinishStatus FinishStatus = null)
        {
                ...
                var QueueWrite = _ThreadManageList.Find(o => o.SlaveConfig.CabinetId == slave.CabinetId);
                    QueueWrite.QueueWrite.Add(
                         new ThreadWriteMsg
                         {
                             FinishStatus = FinishStatus,
        ...
        private void ThreadReadGratingMsg(ushort[] registers, SlaveConfig slave)
        {
            try
            {
                ...
                for (int i = 0; i < registers.Length; i++)
                {
                    ...
                    {
                        //SaveErrLogHelper.SaveErrorLog(string.Empty, "阻挡");
                        bool isBlock = false;
                        foreach (ThreadSortOrder ThreadSortOrder in _ThreadSortOrderManager.Get())
                        {
